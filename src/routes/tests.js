...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const { data: test, error } = await supabase.from('tests')
      .insert({ teacher_id: req.teacher.id, title: title.trim(), description: description.trim(), in_app: inApp, code, status })
      .select().single();
...
router.get('/by-code/:code', async (req, res) => {
  ...
  res.json({
    test: {
      ...test,
      questions: test.questions.map(({ id, type, text, points, options }) => ({ id, type, text, points, options }))
    }
  });
});
